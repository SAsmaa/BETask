...
const hasBalance = async (req, res,next) => {
    ...
    const [results,metadata] = await sequelize.query(
        ...
        +" and p.balance >= j.price"
      )
  
    if(!results[0].ClientId) return res.status(401).send("Not Enough Balance")
    req.result = results
    await t.commit();
}
catch (error) {
...
const creditContrBalance = async (req,res,next) => {
    ...
    try {
    const [results,metadata] = await sequelize.query(
        "update Profiles  set balance = p.balance + " +  req.result[0].price +" where id = "+req.result[0].ContractorId
      ).catch(function (err) {
        console.log(err)
...
const updateJob = async (req,res,next) => {
    ...
    try {
    const [results,metadata] = await sequelize.query(
        "update jobs set paid = 1 where id = " + req.result[0].id
      ).catch(function (err) {
        console.log(err)
        return res.status(501).end;
      });
